...
#
#
# version 1.06  build 2025.03.01
# Author: James Kirchner, ETH Zurich
#
# Copyright (C) 2025 ETH Zurich and James Kirchner
# Public use is permitted under GNU General Public License 3 (GPL3); for details see <https://www.gnu.org/licenses/>
# 
#
# READ THIS CAREFULLY:
...
#
# ALSO READ THIS:
# These scripts implement ensemble rainfall runoff analysis as presented in J.W. Kirchner, Characterizing nonlinear, 
#     nonstationary, and heterogeneous hydrologic behavior using Ensemble Rainfall-Runoff Analysis (ERRA): proof of concept,
#     Hydrology and Earth System Sciences, 28, 4427-4454, https://doi.org/10.5194/hess-28-4427-2024, 2024. (hereafter denoted K2024).
#     Users should cite that paper.
#
...
#### The functions buildxx, calc_pred, and calc_ypred have been translated from Rcpp to base R, so that users do not need to install a C++ compiler ####
#### Execution can be substantially slower, however, particularly for large problems ####
#////////////////////////////////////////////////////////////////////////////////////////////////
#////////////////////////////////////////////////////////////////////////////////////////////////
...
# this is the vanilla R version of the Rcpp function calc_pred in ERRA_v1.0.R
calc_pred <- function (x, y, beta, nx, n, nlag, h)    #THIS FUNCTION IS FOR USE IN SOLVAR, NOT IRF
{
  #x is a matrix of nx columns and n rows (which is handled here as a vector, for consistency with the Rcpp version)
  #nlag is the number of lags in the xx matrix; h is the number of lags that we add from the y vector
  
  x <- as.vector(x)
  pred <- rep(beta[length(beta)], times=(n-nlag+1))  # declare and initialize the target vector to the constant term
  
  # step through columns of x and step through lags
  for (i in 0:(nx-1)) for (lag in 0:(nlag-1)) pred = pred + beta[i*nlag+lag+1]*x[(i*n+nlag-lag):(i*n+n-lag)];
  
  # now do lags of y for AR correction
  if (h>0) for (i in 0:(h-1)) pred = pred + beta[nx*nlag+i+1]*y[(nlag-i-1):(n-i-1)];
...
# this is the vanilla R version of the Rcpp function calc_ypred in ERRA_v1.0.R
calc_ypred <- function (x, beta, nx, n, m, h)    #THIS FUNCTION IS FOR USE IN IRF, NOT SOLVAR
{
  #x is a matrix of nx columns and n rows (which has been passed here as a vector)
  #nlag is the number of lags in the xx matrix; h is the number of lags that we add from the y vector
  
  nlag <- m+h+1
  pred <- rep(0, times=(n-nlag+1))  # declare and initialize the target vector
  for (i in 0:(nx-1)) for (lag in 0:m) pred <- pred + beta[(i*(m+1)+lag+1)]*x[(i*n+nlag-lag):(i*n+n-lag)]  #+1's relative to Rcpp are because C vectors start at 0 and R vectors start at 1
...
buildxx <-                        # this is the vanilla R version of the Rcpp function buildxx in ERRA_v1.0.R
  function(x, y, nx, n, nlag, h, chunktop, chunkht)
    
  {
    # x is an matrix of nx columns and n rows
    # nlag is the number of lags in the x matrix
    # h is the number of lags that we add from the y vector
    
    xx <- matrix(ncol=(nx*nlag+h+1), nrow=chunkht)  # declare and inititalize the target matrix
    
    for (i in 1:nx) {
      for (lag in 1:nlag) {
        xx[ ,((i-1)*nlag+lag)] <- x[(1+(chunktop-lag)):(chunktop-lag+chunkht) , i]
      }
    }
    
    # add lags of y for AR correction
    if (h>0) for (i in 1:h) xx[ ,(nx*nlag+i)] <- y[(chunktop-i):(chunktop-i-1+chunkht)]
    
    xx[ , NCOL(xx)] <- 1    #a column of 1's for the constant term
    ...
    if (!is.null(xx)) {                                        
      ...
      repeat{
        ...
        if (sum(sqwt[chunktop:chunkend])>0) {                      # if all of the rows in this chunk have weight of zero then we can skip this one
          xx <- buildxx(x, y, nx, n, nlags, h, chunktop, chunkht)  # here we build the xx matrix
          # xx <- matrix(xx, nrow=chunkht) # this is only necessary if we get our xx matrix from Rcpp (which exports xx as a vector)
...
BrokenStickRobustSolvAR <-
    ...
    
  {
    ...
    for (k in 1:nk) {
      xf <- as.matrix(stats::filter(x, filter=tauwt[[k]], method="c", sides=1))  # now apply convolutional filter for each knot to x's (xf is filtered x) -- note that doing this by FFT is MUCH slower!!
    ...
    repeat{
      ...
      for (j in 1:nx) {   # now step through each of the x's
        ...
        pred <- pred + as.vector(stats::filter(x[, j], filter=allb[ ,j], method="c", sides=1))  # use these interpolated b's to predict y (and keep them for later, for AR correction of beta...)
      } # next j
    ...
    pacf <- wtd.pacf(e, rw*orig_wt)           # we need to calculate *weighted* partial autocorrelations... using homebaked function above                             
    resid.pacf <- pacf$pacf
    resid.acf <- pacf$acf
...

  {
    ...
    repeat {                           # start the AR selection loop
      ...
      if (robust & firstpass) {  
        ...
        if (s1$resid.acf[1]>ser.corr.warn) warning("Residuals have lag-1 serial correlation=",  round(resid.acf[1], 4), "  Consider aggregating time steps.")      
      ...
      if (h >= max.AR) {                                                                         # if we have reached the maximum AR order allowed
        warning(paste0("Maximum AR order of ", max.AR, " reached in iterative loop. Consider aggregating time steps."))            # print a warning
        {break}                                                                                  # and exit
      }
                      ...
                      ser.corr.warn = 0.99)
{
  ...
  if ((nk>0)&&(nk<m+1)) nbeta <- nk                                # nbeta is the number of coefficients for each x and each xknot (the total number of coefficients is nx*nxk*nbeta)
  else nbeta <- m+1                                                # or for each x and each of m+1, if nk>m+1 so we have reverted to fixed lags
...
peakstats <- function(lagtime, bb, se, Qavgd=TRUE, rc_pos=TRUE ) {
  # takes as input:
  ...
  # rc_pos          boolean flag: if TRUE, negative values will be set to zero when calculating runoff coefficients       
  ...
  if ((peakbb <=0) | is.infinite(ll) | is.infinite(ff)) {                               # if no feasible solution
    ...
    ff <- max(ff, pk-3)    # new in version 1.01: limit fitting to maximum 3 points before and after the highest point
    ll <- min(ll, pk+3)
  ...
  if (rc_pos==TRUE) rc <- sum(pmax(bb,0) * rcwt)        # integral of linear interpolation between points
  else rc <- sum(bb * rcwt)                            # integral of linear interpolation between points
...
make_sets <-
  function(n , np=1, wt=rep(1, n), split_params = NULL, p_label, na_flag) {
    ...
    # na_flag        a Boolean vector indicating time steps at which precipitation will be unusable (for exclusion in setting ranges of splitting parameters)
    ...
    for (i in 1:n_crit) {
      ...
      if ( (max(breakpts[[i]])<1) && (pct_breakpts[i]==TRUE) ) {
        warning("pct_breakpts is TRUE, but the supplied breakpts xknots appear to be fractions (max is ", max(breakpts[[i]]) ,"). These were converted to percentiles.")
        breakpts[[i]] <- breakpts[[i]]*100  # if pct_breakpts is TRUE but max(breakpts)<1, assume users have mistakenly specified quantiles and recalculate as percentiles
      }
    } 
    ...
    for (ii in 1:np) {  # step through each precipitation series in order
      ...
      for (i in 1:n_crit) {   # step through each criterion in order
        ...
        if (NCOL(cr)>1) cr <- cr[,ii, drop=FALSE] # if there are multiple columns in this criterion matrix, extract the one that corresponds to this precipitation series
        ...
        if (pct_breakpts[[i]]==FALSE) {   # check that breakpoint values are not out of range
          ...
          if ( (min(bp) <= min(cr, na.rm=TRUE) ) | (max(bp) >= max(cr, na.rm=TRUE)) )
            stop("Fatal error in ERRA while splitting precipitation: one or more breakpoints are outside range of criterion variable")
        } else {
          if (th>max(cr, na.rm=TRUE)) stop("Fatal error in ERRA while splitting precipitation: all values of criterion variable lie below specified threshold")
        ...
        if (by_bin[[i]]) {                        # if breakpoint quantiles are nested into (i.e., applied separately to) each set of previous breakpoint bins
          for (j in 1:n_sets) {                   # then cycle through the extant sets from previous criteria
            ...
            crss <- cr[((cr>th) & (old_sets==j) & (na_flag==FALSE))]     # criterion subset (above threshold, only values within the current set)
            bp <- quantile(crss, probs=qbp/100.0, na.rm=TRUE, type=5)    # convert quantile breakpoints to criterion values
            crss <- cr[((old_sets==j) & (na_flag==FALSE))]               # different criterion subset (NO threshold, only values within the current set)
          ...
          if (pct_breakpts[[i]]==TRUE) bp <- quantile(cr[((cr>th) & (na_flag==FALSE))], probs=qbp/100.0, na.rm=TRUE, type=5)  # convert quantiles to criterion values
...
agg_data <- function(x , agg) {
  ...
  if(!is.null(x)) {
    if (is.list(x)) for (i in 1:length(x)) {   # if x is a list we need to work through each element individually
      ...
      vecflag <- is.vector(x[[i]])
      x[[i]] <- as.matrix(x[[i]])
      n <- NROW(x[[i]])
      d <- (1:n) %% agg           # d is row number modulo agg
      xf <- as.matrix(stats::filter(as.numeric(x[[i]]), fv, sides=1))  # average the last "agg" values
      if (is.logical(x[[i]])) x[[i]] <- as.logical(round(xf[d==0, , drop=FALSE]))  # aggregate logical values using majority rule   
      else x[[i]] <- xf[d==0, , drop=FALSE]  # aggregate each criterion variable (by rows) 
      if (vecflag) x[[i]] <- as.vector(x[[i]])  #if x[[i]] was originally a vector, make its aggregation a vector too
      ...
      if (is.logical(x)) x <- as.logical(round(xf[d==0, , drop=FALSE]))  # aggregate logical values using majority rule 
      else x <- xf[d==0, , drop=FALSE]  # aggregate each criterion variable (by rows) 
      if (vecflag) x <- as.vector(x)    # if x was originally a vector, make its aggregation a vector too
...
ERRA <-
...

  {
    ...
    # rc_pos          boolean flag controlling whether negative values are set to zero in calculations of runoff coefficients (default is TRUE)
    ...
    # p and q can have missing values (NA), but the time series must be continuous and evenly spaced (and must of course be the same for both p and q).
    ...
    #           peakht                   peak of RRD by quadratic fitting to uppermost 20% of RRD (or 3 points on either side of highest value)
    #           peakht_se                standard error
    #           tpeak                    time of peak by quadratic fitting to uppermost 20% of RRD (or 3 points on either side of highest value)
    ...
    #           tpeak                    time of peak by quadratic fitting to uppermost 20% of NRF (or 3 points on either side of highest value)
    #           tpeak_se                 standard error
    ...
    if ((robust==TRUE) && is.null(xknots)) {  
      warning(paste0("Robust estimation has been selected together with xknots==NULL.  This may result in underestimation of runoff response due to downweighting of high precipitation inputs.  Users are generally advised to use robust estimation only when xknots are also used to account for nonlinear response to precipitation intensity."))
        ...
        # in setting xknots and making precipitation subsets, we need to exclude P values that will be unusable later (otherwise we could have too few nonzero values in some columns)
    na_flag <- as.integer(is.na(rowSums(p, na.rm=FALSE)) | is.na(q) | !Qfilter)       # boolean vector of rows where any of p_subsets or q is missing, or q is excluded
    if (sum(na_flag)>0) {                                                          # if there are any of these, we need to propagate them backwards
      ...
      p_flag <- stats::filter(as.integer(is.na(rowSums(p, na.rm=FALSE))), missing.cancels, sides=1, circular=TRUE)  # also need to convolve missing p values forward
    ...
    ss <- make_sets(n=n, np=np, wt=p, split_params=split_params, p_label=p_label, na_flag=na_flag)   # apply criteria to define sets of time steps
      ...
      if (substr(xknot_type,1,1)=="v") pct_xknots=FALSE       # xknot_type=="values"
        ...
        for (j in 1:NCOL(p_subsets)) {
          ...
          for (i in 1:nknots) {
            ...
            if (is.null(xx[mc]) || (xx[mc]==0)) {# note that we use warning() rather than stop() here, so that we can return a null result and thus any calling routine cannot continue as normal
              warning(paste0("One or more precipitation subsets has less than ", mincount, " nonzero precip values between a pair of xknots.  Stopping with null result."))
              return()
            }
        ...
        # and now reset pct_xknots, because xknots are now values
        pct_xknots <- FALSE                                        
      } # end last else
      
     }}}
      
      if ( (pct_xknots!=FALSE) && (max(xknots)<1) ) { 
        warning("xknot_type was specified as ", xknot_type ,", but the supplied xknots appear to be fractions (max is ", max(xknots) ,"). These were converted to percentiles.")
        xknots <- xknots*100  ## if pct_xknots is not FALSE but max(xknots)<1, assume users have mistakenly specified quantiles and recalculate as percentiles
      }
    } # end if
