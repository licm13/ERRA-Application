...
antQparams <- list(crit = list(q) ,                  # must be LIST of criterion variables; each must be of same length as P
                   ...
                   pct_breakpts = c(FALSE) ,             # boolean vector of same length as crit: TRUE means breakpoints are raw values, FALSE means breakpoints are percentiles
...
zz <- ERRA(p=p, q=q, xknots=c(4, 20), xknot_type="even", m=1000, nk=40, h=3, split_params=antQparams)   # here we fit to the entire time series
...
fileID <- "Fitting demo Hafren76-85"   # fitting to Hafren 1976-1985



zz <- ERRA(p=p, q=q, xknots=c(4, 20), xknot_type="even", m=1000, nk=40, h=3, split_params=antQparams, Qfilter=ifelse((dat$yr<=1985), 1, 0))
...
with(zz$Qcomp, {
  NSE <- 1-var(Qresidual, na.rm=TRUE)/var(Q[!is.na(Qresidual)])
  NSE_in_sample <-  1-var(Qresidual[(weight>0)], na.rm=TRUE)/var(Q[(weight>0)&(!is.na(Qresidual))])
  NSE_out_of_sample <-  1-var(Qresidual[(weight==0)], na.rm=TRUE)/var(Q[(weight==0)&(!is.na(Qresidual))])
  print(paste0("NSE ", NSE))
  print(paste0("in-sample NSE ", NSE_in_sample))
  print(paste0("out-of-sample NSE ", NSE_out_of_sample))
})
...
fileID <- "Fitting demo Hafren01-10"    # fitting to Hafren 2001-2010

zz <- ERRA(p=p, q=q, xknots=c(4, 20), xknot_type="even", m=1000, nk=40, h=3, split_params=antQparams, Qfilter=ifelse((dat$yr>=2001), 1, 0))
